...
def category_list(request, category_id):
    ...
    page_number = request.GET.get('page')   # http://assas.co/?page=1 (页码)
    page_obj = paginator.get_page(page_number)
    context = {'category': category, 'page_obj': page_obj,'user': user}
...
def post_detail(request, post_id):
    ...
    context = {'post': post, 'prev_post': prev_post, 'next_post': next_post,'user':user}
...
def archives(request, year, month):
    ...
    page_number = request.GET.get('page')   # http://assas.co/?page=1 (页码)
    page_obj = paginator.get_page(page_number)
    context = {'page_obj': page_obj, 'year': year, 'month': month,'user':user}
